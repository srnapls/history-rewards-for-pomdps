...
\begin{definition}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item $s_{I,\mathcal{F}} = \langle s_I, \delta(n_I, O(s_I))\rangle$, the initial state;\todo{explain this}
...
Now we can extend this induced POMDP $\mathcal{M}_\mathcal{F}$ as presented in \defref{d:extended_pomdp}, yielding $\widetilde{\mathcal{M}_\mathcal{F}}$, with the adjusted reward function in which we obtain the related reward when we enter the final state through action \texttt{end}.
