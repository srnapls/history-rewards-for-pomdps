...
\begin{itemize}
	...
	\item $R:(S\times A)^*\to\mathbb{R}$ - which looks at the finite (sub)trajectory without the last obtained state, or;
...
For simple reward functions, we can solve the problem of how to obtain a policy for the optimal expected reward iteratively, using the Bellman equation \todo{quote, etc}

We introduce a discounting factor $\gamma\in[0,1]$
\towrite{why do we use gamma, talk about model mortality etc}
...
The optimal expected reward at time $n$ can be calculated iteratively as follows:
