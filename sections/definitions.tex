...
Based on the definition as presented in \cite{p:mealy}.
\begin{definition}
	A Mealy machine is a tuple $(Q,q_0,\Sigma,O,\delta,\lambda)$ where
	\begin{itemize}
		...
		\item $\lambda : Q\times \Sigma\to O$, the output transition function.
	\end{itemize}
\end{definition}
\subsection*{Example}
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=3cm,on grid,auto]
		...
		\path[->] 	(1) edge[loop above] 	node [above] {$a/1$}			(1)
						edge[bend right] 	node [below] {$b/0$}			(2)
					(2) edge[bend right] 	node [above] {$a/0$}			(1)
						edge[loop above] 	node [above] {$b/1$}			(2);
					
				
	\end{tikzpicture}
	\label{fig:simple_rc}
	\caption{Simple Mealy machine}
\end{figure}

\section{Reward machine}
\todo{This is obsolete, since we make the Reward Controller a specific instance of a Mealy Machine where the output character is just a subset of $\mathbb{R}$}
...
\todo{This can stay, maybe slighty adjusteD?}
